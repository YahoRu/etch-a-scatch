...
function changeColor(e) {
  if (e.target.value === "rainbow") {
    colorToDrawWith =
      "linear-gradient(to bottom, red 0%, orange 15%, yellow 30%, green 45%, blue 60%, #ff99cc 100%)";
...
function paintOverBlock(e) {
  if (!mouseButtonDown && e.type === "mouseover") {
    return;
  }

  if (colorInRainbowMode) {
    e.target.style.backgroundColor = randomRGBColor();
  } else {
    e.target.style.backgroundColor = colorToDrawWith;
  }
}
...
function createTable(e) {
  ...
  if (e.target.value !== undefined && e.target.value !== BUTTON_HAS_NO_VALUE) {
    elementsInRow = e.target.value;
  }
